...
def kmeans(points_df, centroids_df, metric):
    ...
    for iter in range(max_iter):
        ...
        for index, point in points_df.iterrows():
            ...
            c_dist = cosine_distance(point.values, centroid)
            sse_val += c_dist ** 2
...
pred_labels_cos, final_centroids_cos, dist_matrix_cos, sse_history_cos = kmeans(data, centroids_df, "cosine")
print("Using Cosine Distance:\n\n\n")

print("\nCluster Assignments:")
print(pred_labels_cos)
print("\nFinal Centroids:")
print(final_centroids_cos)
# print("\nSSE per iteration:")
# print(sse_history)
print(f"\nFinal SSE: {sse_history_cos[-1]}")
...
# accuracy for cosine
acc_cos, map_cos = kmeans_acc(true_labels, pred_labels_cos)
print(f"Cosine Accuracy: {acc_cos}\n\n")
print("Mapping of cluster - label:", map_cos)