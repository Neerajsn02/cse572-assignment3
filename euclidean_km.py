...
def kmeans(points_df, centroids_df, metric):
    ...
    for iter in range(max_iter):
        ...
        for index, point in points_df.iterrows():
            ...
            # if metric == "euclidean":
            # sse_val += np.sum((point.values - centroid) ** 2) # sum the squared error for euclidean
            e_dist = euclidean_distance(point.values, centroid)
            sse_val += e_dist ** 2

        sse.append(sse_val)

        # Stopping condition chekcs
        #############################################################################################################
        ...
        # if len(sse)>3:
        #     if sse[-1] > sse[-2]:
        #         print(f"sse val in iteration {iter} increased voer last iteration")
        #         sse.pop()
        #         break
...
pred_labels_euc, final_centroids_euc, dist_matrix_euc, sse_history_euc = kmeans(data, centroids_df, "euclidean")
print("Using Euclidean Distance:\n\n\n")

print("\nCluster Assignments:")
print(pred_labels_euc)
print("\nFinal Centroids:")
print(final_centroids_euc)
# print("\nSSE per iteration:")
# print(sse_history)
print(f"\nFinal SSE: {sse_history_euc[-1]}")
...
acc_euc, map_euc = kmeans_acc(true_labels, pred_labels_euc)
print(f"Euclidean Accuracy: {acc_euc}\n\n")
print("Mapping of cluster - label:", map_euc)